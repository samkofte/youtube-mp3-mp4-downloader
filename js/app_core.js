...
const app = {
    ...
    async openVideo(id) {
        ...
        try {
            ...
            document.getElementById('videoChannel').textContent = details.author || 'Unknown Channel';
            document.getElementById('videoStats').textContent = `${ui.formatViews(details.view_count || 0)} views`;
            document.getElementById('channelAvatar').textContent = (details.author || 'C').charAt(0).toUpperCase();
            
            // Description rendering (default fallback if missing)
            document.getElementById('videoDescription').textContent = details.description || 'No description available.';
            
            // Set iframe embed source
